 ...
 */
void
normal_cmd(oap, toplevel,event)
oparg_T	*oap;
int		toplevel UNUSED;	/* TRUE when called from main() */
NSEvent *event;
{
    static cmdarg_T	ca;			/* command arguments */
    int		c;
...
//#endif
    ...
    // character from Keyboard event 
    c = (int)[event ASCIIValue];
    //    LANGMAP_ADJUST(c, TRUE);

    NIF_INFO(@"opcount--- %d",opcount);
    NIF_INFO(@"Input Char ASCII %d",c);
...
cmdarg_T	*cap;
{
    ...
    {
            /*
             * just have got a 'g', the cap->nchar == 0
             * except nchar; ignore it
             */
        case 0:
            NIF_INFO(@"");
            break;
    ...
    if ((nv_cmds[idx].cmd_flags & NV_NCH)
	    && (((nv_cmds[idx].cmd_flags & NV_NCH_NOP) == NV_NCH_NOP
            ...
            || (cmdargs.cmdchar == 'q'
                ...
                && !Recording
                && !Exec_reg)
            || ((cmdargs.cmdchar == 'a' || cmdargs.cmdchar == 'i')
                && (oap->op_type != OP_NOP
#ifdef FEAT_VISUAL
                    || VIsual_active
#endif
